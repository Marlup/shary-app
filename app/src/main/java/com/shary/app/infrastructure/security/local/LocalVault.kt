 ...
 *   master = KDF(email, passwordChars, appId)  // 32B
...
class LocalVault(
    ...
    /** Derive a per‑purpose local key from email+password. */
    fun deriveLocalKey(
        email: String,
        password: CharArray,
    ...
    fun encryptCredentials(
        ...
        localKey: ByteArray,
        json: JSONObject,
        aad: ByteArray? = e.encodeToByteArray()
    ): ByteArray = encryptGeneric(
    ...
    private fun encryptGenericByDerivation(
        ...
        val key = deriveLocalKey(e, p, purpose)
    ...
    fun encryptByDerivation(
        ...
        purpose: Purpose,
        aad: ByteArray?
    ): ByteArray = encryptGenericByDerivation(plain, e, p, purpose.code, aad)
    ...
    fun decryptByDerivation(
        ...
        purpose: Purpose,
        aad: ByteArray?
    ): ByteArray = decryptGenericByDerivation(blob, e, p, purpose.code, aad)

    // -------------------- Back‑compat String <-> Base64 wrappers --------------------
    // Note: these DO NOT use AAD to preserve legacy behavior.

    private fun encryptToB64ByDerivation(plain: String, e: String, p: CharArray, purpose: String, aad: ByteArray?): String {
        val key = deriveLocalKey(e, p, purpose)
    ...
    private fun decryptFromB64ByDerivation(b64: String, e: String, p: CharArray, purpose: String, aad: ByteArray?): String {
        val blob = Base64.decode(b64, Base64.DEFAULT)
        val (iv, body, tag) = unpack(blob)
        val key = deriveLocalKey(e, p, purpose)
    ...
    fun encryptToStringByDerivation(
        ...
        purpose: String,
        aad: ByteArray?
    ): String = encryptToB64ByDerivation(plain, e, p, purpose, aad)
    ...
    fun decryptToStringByDerivation(
        ...
        purpose: String,
        aad: ByteArray?
    ): String = decryptFromB64ByDerivation(b64, e, p, purpose, aad)
    ...
    fun encryptCredentialsByDerivation(
        ...
        json: JSONObject,
        aad: ByteArray? = e.encodeToByteArray()
    ): ByteArray = encryptGenericByDerivation(
        json.toString().encodeToByteArray(),
        e,
        safe.toCharArray(),
