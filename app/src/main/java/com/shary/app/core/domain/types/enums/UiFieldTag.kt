...
sealed class UiFieldTag(val name: String) {
    ...
    companion object {
        ...
        fun fromString(tagName: String): UiFieldTag = when (tagName.lowercase()) {
            "taxes" -> Taxes
            "home" -> Home
            "personal" -> Personal
            "bank" -> Bank
        ...
        fun toString(tag: UiFieldTag): String = when (tag) {
            is Custom -> tag.customName
            else -> tag.name
        }
    }
}
...
fun tagColor(tag: UiFieldTag): Color = TagColorByKey[tag.key] ?: DefaultUnknownTagColor
