...
fun AddCopyFieldDialog(
    ...
    var key by remember { mutableStateOf(targetField.key + " - Copy") }
    val predefinedKeys = PredefinedKey.entries.map { it.key }
    var keyAlias by remember { mutableStateOf(targetField.keyAlias) }
    var value by remember { mutableStateOf(targetField.value) }
